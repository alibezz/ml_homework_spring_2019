...
\noindent Assuming that the weights of the rocks ($w_1$, $w_2$, $w_3$) are conditionally independent, we can rewrite the numerator in Equation \ref{eq1} as
...
\noindent Let us tackle this numerator first. Previously, we calculated that $P(L = A) = 0.2$. To compute the other factors, we use the probability density~\footnote{I created a python function to automate the calculation of pdfs, and its code is in the appendix of this homework.} of the 
...
\noindent As for the denominator in Equation~\ref{eq1}, it can be rewritten as:
...
\noindent And we just have to compute the terms that involve $L = B$. We again also assume conditional independency and use the probability density of the 
...
\noindent This posterior probability indicates that, given the prior and the evidence, it is not likely that the rocks were collected in Location A.
